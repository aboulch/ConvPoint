...
def main():
    ...
    if convertcompo:
        ...
        for fname in filelist_test:
            print(fname)
            filename_composite = os.path.join(args.composite, 
                                fname.replace("_voxels.npy","_composite.txt"))
            filename_save = os.path.join(args.composite, 
                                fname.replace("_voxels.npy","_composite"))
            np.save(filename_save, np.loadtxt(filename_composite)[:,:2].astype(np.uint8))
        exit()
    ...
    if savenumpy:
        for fname in filelist_train:
            ...
            np.save(os.path.join(args.rootdir, fname), np.loadtxt(os.path.join(args.rootdir, fname_txt)).astype(np.float16))
        exit()
    ...
    if not args.test:
        ...
        ds = PartDataset(filelist_train, args.rootdir, None,
