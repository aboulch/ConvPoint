...
def main():
    ...
    parser.add_argument('--rootdir', '-g', help='Path to ground truth folder', required=True)
    parser.add_argument('--preddir', '-p', help='Path to prediction folder', required=True)
    parser.add_argument('--part_avg', '-a', action='store_true', help='Use part level average')
    args = parser.parse_args()
    print(args)

    args.folder_gt = os.path.join(args.rootdir, "test_label")
    ...
    for category in categories:
        category_folder_gt = os.path.join(args.folder_gt, category)
        category_folder_pred = os.path.join(args.preddir, category)
